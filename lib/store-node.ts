...
import { db, transaction } from "./db-node";
import {
  initialGame,
  normalizeGame,
  reduceGame,
  ...
  return transaction(() => {
    ...
    if (["marketList", "marketBuy", "marketCancel"].includes(action.type)) {
      ...
      if (action.type === "marketList") {
        if (game.run)
          throw new RuleError("탐사 중에는 매물을 등록할 수 없습니다.");
        ...
        if ((game.materials[listingItem] || 0) < listingQuantity)
      ...
      const listing = db
        .prepare("SELECT * FROM market_listing WHERE id=? AND status='open'")
        .get(action.id) as
        | {
            id: string;
            seller_account: string;
      ...
      if (action.type === "marketCancel") {
        ...
        if (game.run)
          throw new RuleError("탐사 중에는 매물을 취소할 수 없습니다.");
        game.materials[listing.item] =
          (game.materials[listing.item] || 0) + listing.quantity;
        game.version++;
        db.prepare(
      ...
      seller.version++;
      db.prepare(
        "UPDATE market_listing SET status='sold',buyer_account=?,updated_at=? WHERE id=?",
      ).run(account, now, listing.id);
      db.prepare(
